...
class UserService extends BaseService {
  ...
  async passwordConfirm(data, config = {}) {
    const response = await this.api.post(`${this.endpoint}/validar-senha`, data, config);
    return response.data;
  }

  async updatePassword(data, config = {}) {
    const response = await this.api.patch(`${this.endpoint}/atualizar/senha`, data, config);
    return response.data;
  }
}
